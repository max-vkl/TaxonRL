...
def compute_score(
    ...
    for reward_input in reward_inputs:
        ...
        (
            main_gt,
            _actual_distance, # This will be an empty string from the new parser
            img1_genus,
            img1_family,
            img1_order,
            img2_genus,
        ...
        # Concrete taxonomy reward based on explicit labels and structural constraints
        taxonomy_score = taxonomic_concrete_reward(
            predict,
            img1_order,
            img2_order,
            img1_family,
        ...
        try:
            ...
            true_prob = float(str(main_gt).strip())
...
def parse_ground_truth_fields(
    ...
    str,  # actual_distance (placeholder, not in new format)
    str,  # img1Genus
    str,  # img1Family
    str,  # img1Order
    str,  # img2Genus
    ...
    """
    Parses fungus taxonomy ground truth fields formatted as:
    'answer;family1;genus1;order1;species1;family2;genus2;order2;species2'
    
    Returns a tuple in the order expected by compute_score:
    (main_gt, _actual_distance_placeholder, img1_genus, img1_family, img1_order, img2_genus, img2_family, img2_order)
    """
    parts = [p.strip() for p in str(ground_truth).split(";")]

    # New format: 'answer;family1;genus1;order1;species1;family2;genus2;order2;species2'
    main_gt = parts[0] if len(parts) >= 1 else ""
    img1_family = parts[1] if len(parts) >= 2 else ""
    img1_genus = parts[2] if len(parts) >= 3 else ""
    img1_order = parts[3] if len(parts) >= 4 else ""
    # parts[4] is species1, which is not used by the current callers
    img2_family = parts[5] if len(parts) >= 6 else ""
    img2_genus = parts[6] if len(parts) >= 7 else ""
    img2_order = parts[7] if len(parts) >= 8 else ""
    # parts[8] is species2, which is not used by the current callers

    # Return in the specific order expected by the unpacking in compute_score
    # (main_gt, _actual_distance, img1_genus, img1_family, img1_order, img2_genus, img2_family, img2_order)
    return (
        main_gt,
        "",  # Placeholder for _actual_distance, which is no longer in GT
        img1_genus,
        img1_family,
        img1_order,
        img2_genus,
...
def taxonomic_concrete_reward(
    ...
    """
    Compare predicted concrete taxonomy names to ground-truth names under hierarchical rules required by
    birds_im_concrete.jinja:

    - <order> must be provided with two names: "order1; order2". Score 1 if both match GT exactly.
    - If GT orders are different, the model should NOT emit <family> or <genus> tags. Absence scores 1; presence scores 0.
    - If GT orders are the same, we evaluate <family>:
        * Score 1 if tag exists and both family names match GT exactly; else 0.
      Then for <genus>:
        * If GT families are different, <genus> should be absent (absence scores 1, presence 0).
        * If GT families are the same, require <genus> to exist and match both names exactly for score 1.

    The final score is the average over evaluated components. Components that cannot be evaluated due to
    missing ground-truth names are skipped from the average.
    """

    def norm(x: str) -> str:
        return (x or "").strip().lower()

    def extract_pair(text: str, tag: str) -> tuple[str | None, str | None]:
        m = re.search(rf"<{tag}>\s*(.*?)\s*</{tag}>", text, re.IGNORECASE | re.DOTALL)
        if not m:
            return None, None
        raw = m.group(1).strip()
        # Expect semicolon-separated names
    ...
    gt_order_1, gt_order_2 = norm(img1_order), norm(img2_order)
    gt_family_1, gt_family_2 = norm(img1_family), norm(img2_family)
    gt_genus_1, gt_genus_2 = norm(img1_genus), norm(img2_genus)

    # Predicted pairs
    pred_order_1, pred_order_2 = extract_pair(predict, "order")
    pred_family_1, pred_family_2 = extract_pair(predict, "family")
    pred_genus_1, pred_genus_2 = extract_pair(predict, "genus")

    # Normalize predicted values if present
    pred_order_1 = norm(pred_order_1) if pred_order_1 is not None else None
    pred_order_2 = norm(pred_order_2) if pred_order_2 is not None else None
    pred_family_1 = norm(pred_family_1) if pred_family_1 is not None else None
    pred_family_2 = norm(pred_family_2) if pred_family_2 is not None else None
    pred_genus_1 = norm(pred_genus_1) if pred_genus_1 is not None else None
    pred_genus_2 = norm(pred_genus_2) if pred_genus_2 is not None else None

    scores: list[float] = []

    # 1) Order name accuracy (require GT availability and predicted presence)
    if gt_order_1 and gt_order_2:
        if pred_order_1 is not None and pred_order_2 is not None:
            scores.append(1.0 if (pred_order_1 == gt_order_1 and pred_order_2 == gt_order_2) else 0.0)
        else:
            scores.append(0.0)

    # Determine GT equality for hierarchy
    gt_order_same = (gt_order_1 == gt_order_2) if (gt_order_1 and gt_order_2) else None
    gt_family_same = (gt_family_1 == gt_family_2) if (gt_family_1 and gt_family_2) else None

    # 2) Family: absence if orders differ; else exact-match if GT available
    if gt_order_same is False:
        # Should not emit <family>
        scores.append(1.0 if (pred_family_1 is None and pred_family_2 is None) else 0.0)
        # Genus should also not be emitted when orders differ (handled below as well)
    elif gt_order_same is True and (gt_family_1 and gt_family_2):
        # Evaluate exact-name match
        if pred_family_1 is not None and pred_family_2 is not None:
            scores.append(1.0 if (pred_family_1 == gt_family_1 and pred_family_2 == gt_family_2) else 0.0)
        else:
            scores.append(0.0)
    # If GT family is missing or GT cannot determine equality, skip family scoring

    # 3) Genus: absence if families differ (or orders differ); else exact-match if GT available
    if gt_order_same is False:
        scores.append(1.0 if (pred_genus_1 is None and pred_genus_2 is None) else 0.0)
    elif gt_order_same is True:
        if gt_family_same is False:
            # Should not emit genus
            scores.append(1.0 if (pred_genus_1 is None and pred_genus_2 is None) else 0.0)
        elif gt_family_same is True and (gt_genus_1 and gt_genus_2):
            if pred_genus_1 is not None and pred_genus_2 is not None:
                scores.append(1.0 if (pred_genus_1 == gt_genus_1 and pred_genus_2 == gt_genus_2) else 0.0)
            else:
                scores.append(0.0)
        # If genus GT missing, skip
